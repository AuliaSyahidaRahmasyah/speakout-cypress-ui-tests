describe("Aduan Bermasalah - Guru", () => {
  it("Tambah Aduan - Entri Bermasalah Baru", () => {
    ...
    cy.get("#txtcarimaster").type("Heroku", { delay: 100 }).click();
    cy.get(':nth-child(2) > .text-decoration-none').eq(0).click({force: true});
    cy.wait(2000);
    // cy.get("#info-ok").click();
    // cy.wait(2000);

    cy.get("#select2-cbojnspelanggaran-container").click();
    cy.get(".select2-search__field").type("contek{enter}");

    cy.wait(1000);

    cy.get("#txtket").type("masalah");
